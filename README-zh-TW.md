...
> **! 注意：當前版本 GreenLuma 不再支援進行遊戲下載，家庭共享的遊戲請不注入啟動 Steam 後下載 !**
...
答：因為 Greenluma 並不開源（至少我沒找到），最多只能解鎖147個是作者寫在程式裡的硬性限制。原因可能是 GreenLuma 需要將你的遊戲 ID 偽裝成其他遊戲 ID 來繞過 Steam 的驗證，但作者只找到了這麼多個能替換的免費遊戲（所有人都有權存取的）。
...
本專案最初是為方便我和朋友連線遊戲而開發的早期作品。後續經歷了長時間的修補，因此程式碼結構較為混亂，許多邏輯屬於臨時解決方案，部分功能尚不健全。開源僅供交流與學習。

此專案僅作為 GreenLuma 的圖形介面工具，與同類專案相比，可能存在以下不足：
*   必須依賴管理員權限執行。
*   可能會引起防毒軟體的誤報。
*   當遊戲檔案過多時，軟體幀數可能會下降。
*   未實現同類產品的部分便捷功能，如：成就管理、遊戲脫殼、停止遊戲更新、D加密繞過、直接使用清單倉庫、網路加速等。

CN-GreenLumaGUI 不以建立一個包羅萬象的軟體為目標，因此後續不會再有功能上的大幅革新，但依然會長期維護，持續進行 Bug 修復、翻譯調整以及跟隨 GreenLuma 的更新。

歡迎各位發布類似功能的作品（我也很期待使用），同時也歡迎對本專案進行修改和重構。
