...
经常玩家庭共享，或者退款过游戏的人可能注意到：如果已经下载了一个游戏，而且你失去了游玩权(可能是你退款了，或者家庭的其他人在玩)的情况下，Steam不会给你提供"开始游戏"的按钮，取而代之的是一个"购买"按钮。

这个软件的真正功能是，将这个"购买"按钮强制变回"开始游戏"按钮，使得你可以强行启动游戏。

如果你根本没有游戏，本地也没有下载好的游戏文件，那么在库里面看不到游戏也是正常的，自然也就看不到这个按钮，同时这个软件的功能就完全无法体现。

如果你觉得这款软件可以无条件让你下载你没有的游戏，那可能你对这款软件的功能存在误解。

另外，在功能生效时，Steam里面和下载有关的功能目前不能工作。对于你自己的游戏，请以正常方式启动Steam后再下载更新。
...
> **! 注意：当前版本GreenLuma不再支持进行游戏下载，家庭共享的游戏请不注入启动Steam后下载 !**
...
~~答：如果偶尔出现该问题。可能是你没有解锁，比如用自己的方式启动的Steam，而没有通过工具启动，要么就是你导入新的清单后忘记了重启Steam。请勾选这个游戏和它的depots，然后用软件启动Steam。~~

~~如果频繁出现该问题，可能是某天Steam的策略有调整，或者软件的解锁功能失效了，需要等待更新。~~
...
#### 问: “用于替换的app池”这个页面是干什么的？

答：解锁的原理是“暂时放弃一个游戏的所有权，来兑换另一个游戏的所有权”——把池中某个app的所有权，暂时让给你想解锁的游戏。所以池中有多少个可用的app，就最多能同时解锁多少个游戏，**可用数量就是解锁上限**（界面右下角显示的那个数字）。

这个页面就是管理这个池子的：

* 池里默认是软件内置的一批免费app（显示为灰色，不可删除，但可以禁用）。默认它们是隐藏的，勾选“显示内置池”才会列出来。
* 想提高上限，就输入你已经拥有（指你自己账号拥有，不包括家庭组其他人拥有的）、但暂时不想游玩的游戏的AppID。每加一个，上限就+1。
* 每一项前面有勾选框，取消勾选即为“临时禁用”，被禁用的项不计入可用数量。
* 可以用筛选下拉框切换“全都显示 / 仅显示没被禁用的 / 仅显示被禁用的”。
* 点“查看对应关系”按钮，会弹窗列出当前配置里“池APPID → 游戏名(游戏ID)”的实际对应关系，缩进的行是DLC。

注意：被放进池子并实际用于替换的游戏，在解锁启动期间你将无法正常游玩它。想玩回来时，禁用或删除该项即可，不影响你对它的真实所有权。

以上机制源于 GreenLuma 本身的实现方式，所有替换池数据均为纯本地数据，不会以任何形式与云端交互。


#### 问: 一堆游戏里面只有个别几个没有解锁成功，怎么办？

答：很可能是内置的APPID池子里有一两个app已经失效了（比如那个免费app被下架、或者不再是所有人都能访问）。失效的池app替换到哪个游戏，那个游戏就解锁不了。

**临时的办法**：先取消前面一两个游戏的勾选，这样后面游戏的解锁顺序就会整体变化，原本对上失效app的游戏就会换到一个正常的app上，你可以先这样凑合玩着。

**更准的办法**：如果你知道是第几个游戏出问题，可以点“用于替换的app池”页面里的“查看对应关系”按钮，找到那个游戏对应的池APPID，然后在列表里手动把这一项**禁用**掉。禁用后它就不再参与替换，后面的项会自动顺延。

（顺带一提，这也是为什么“DLC只解锁了一部分”有时会表现得像在某一行截断——道理是一样的。）
...
答：因为GreenLuma需要将你的游戏伪装成其他游戏的id来骗过Steam的验证，能用来替换的免费游戏(所有人都有权访问的)数量有限，所以存在某个上限。软件界面右下角显示的就是当前实际的上限，它等于“用于替换的app池”中可用的数量，你可以在那个页面里自己增删和禁用。
...
GreenLuma_2026_x64.dll (版本GreenLuma_2026_1.8.7)
...
[2] 替换dll文件: 将你从官方论坛解压出来的 GreenLuma_2026_x**.dll 文件放入 override 文件夹下，不要改名
...
本项目最初是为方便我和朋友联机游戏而开发的早期作品。后续经历了长时间的修补，因此代码结构较为混乱，许多逻辑属于临时解决方案，部分功能尚不健全。开源仅供交流与学习。

此项目仅作为 GreenLuma 的图形界面工具，与同类项目相比，可能存在以下不足：
*   必须依赖管理员权限运行。
*   可能会引起杀毒软件的误报。
*   当游戏文件过多时，软件帧数可能会下降。
*   未实现同类产品的部分便捷功能，如：成就管理、游戏脱壳、停止游戏更新、D加密绕过、直接使用清单仓库、网络加速等。

CN-GreenLumaGUI 不以创建一个包罗万象的软件为目标，因此后续不会再有功能上的大幅革新，但依然会长期维护，持续进行 Bug 修复、翻译调整以及跟随 GreenLuma 的更新。

欢迎各位发布类似功能的作品（我也很期待使用），同时也欢迎对本项目进行修改和重构。
